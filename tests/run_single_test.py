...
def correct_output(expected: str, actual: str) -> bool:
    ...
    for i in range(len(actual_lines)):
        if "<...>" in expected_lines[i]:
            ...
            if not (actual_lines[i].startswith(pre_ex) and actual_lines[i].endswith(post_ex)):
                return False
...
def test_runner(directory_pair: Tuple[str, str], capsys):
    ...
    analysis_classes = getmembers(module, lambda c: isclass(c) and issubclass(c, BaseAnalysis))
    selected_hooks = get_hooks_from_analysis([f"{module_prefix}.analysis.{ac[0]}" for ac in analysis_classes])
