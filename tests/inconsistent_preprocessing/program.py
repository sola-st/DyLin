...
def split_afterwards():
    ...
    X_train, X_test, y_train, y_test = train_test_split(X_transformed, y, test_size=0.4, random_state=random_state)
...
def access_attr():
    ...
    X_Test = pd.DataFrame(
        [[1.0, 2.0], [4.0, 8.0], [0.0, 8.0]], index=['cobra', 'viper', 'sidewinder'], columns=['max_speed', 'shield']
    )
    y = np.array([0, 0, 0])
...
def access_attr_bad_transformation():
    ...
    X_Test = pd.DataFrame(
        [[float("NaN"), 20.0], [4.0, 8.0], [0.0, 8.0]],
        index=['cobra', 'viper', 'sidewinder'],
        columns=['max_speed', 'shield'],
    )
    y = np.array([0, 0, 0])
