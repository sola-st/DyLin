...
class StringConcatAnalysis(BaseDyLinAnalysis):
    ...
    def add_assign(self, dyn_ast: str, iid: int, left: Any, right: Any) -> None:
        ...
        self._check(dyn_ast, iid, left(), right)
    ...
    def write(self, dyn_ast: str, iid: int, old_val: Any, new_val: Any) -> None:
        # old_val is a list of lambdas which reference the old value
        # TODO you can assign to tuples, check through all of them
        if len(old_val) == 1:
            old_val = old_val[0]
        else:
            return

        # check if variable has already been assigned before - if not we don't have to check anyway
        if callable(old_val):
            try:
                old_val = old_val()
            # do pokemon catching here because many things can go wrong of variable or key in variable
        ...
        if isinstance(old_val, type("")) and isinstance(new_val, type("")):
            if (
                self.last_add_operation is not None
                and self.last_add_operation["iid"] == iid - 1
            ):
                if new_val == self.last_add_operation["result"]:
                    self._check(dyn_ast, iid, old_val, new_val)
